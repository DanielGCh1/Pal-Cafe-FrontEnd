...
export default function CreateIngredient() {
    ...
    useEffect(() => {
        if (isUndefinedOrNull(params.id)) {
            setIngredient({
                ing_nombre: "", ing_descripcion: "",
                ing_precio: 0, ing_tipo_unidad: "Gramos", ing_cantidad: 0,
                ing_imagen: "", ing_existencias: "0"
            });
            setCreateOrEdit("Crear Ingrediente");
        }
        if(!isUndefinedOrNull(params.id) && isUndefinedOrNull(ingredient)) {
            ...
            setCreateOrEdit("Editar Ingrediente");
        }
    }, []);
    return <>
